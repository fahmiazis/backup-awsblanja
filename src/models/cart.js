...
module.exports = {
  ...
  getOrderDetails: (id, cb) => {
    db.query(`SELECT order_details.id, order_details.order_id, order_details.product, order_details.quantity, order_details.price, order_details.total_price, order_details.user_id, order_details.product_id, product_images.url FROM order_details INNER JOIN product_images ON order_details.product_id=product_images.product_id WHERE order_id=${id}`, (_err, result, _field) => {
      cb(result)
    })
  },
  ...
  createOrderDetails: (arr, cb) => {
    db.query(`INSERT INTO order_details (user_id, product_id, quantity, price, total_price, product, cart_id) VALUES (${arr[0]}, ${arr[1]}, ${arr[2]}, ${arr[3]}, ${arr[4]}, '${arr[5]}', ${arr[6]})`, (_err, result, _fields) => {
      cb(result)
    })
  },
  getCartModel: (arr, cb) => {
  ...
  deleteOrderDetail: (id, cb) => {
    const query = `DELETE FROM order_details WHERE cart_id=${id}`
    db.query(query, (_err, result, _fields) => {
      cb(result)
    })
  },
  ...
  updateOrder: (data, id) => {
    return new Promise((resolve, reject) => {
      db.query('UPDATE order_details SET ? WHERE cart_id = ?', [data, id], (_err, result, _field) => {
        if (_err) {
          reject(_err)
        } else {
          resolve(result)
