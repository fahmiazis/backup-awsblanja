...
const { createCondModel, getDetailCartModel, getCartModel, getCartModel1, deleteCartModel, deleteCartModel1 } = require('../models/conditions')

module.exports = {
  createCond: (req, res) => {
    const { name } = req.body
    if (name) {
      createCondModel([name], result => {
        res.send({
          success: true,
          data: 'condition has been added'
        })
      })
    } else {
      res.send({
