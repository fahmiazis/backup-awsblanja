...
const { createItemModel, getDetailItemModel, getItemModel, getItemModel1, updateItemModel, updateItemModel1, updatePartialItemModel, deleteItemModel, deleteItemModel1 } = require('../models/items')

module.exports = {
  createItem: (req, res) => {
    const { name, price, description } = req.body
    if (name && price && description) {
      createItemModel([name, price, description], res => {
        res.status(201).send({
          success: true,
  ...
  getItem: (req, res) => {
    ...
    getItemModel([searchKey, searchValue, limit, page], (err, result) => {
      if (!err) {
        const pageInfo = {
          count: 0,
          pages: 0,
